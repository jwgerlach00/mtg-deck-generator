...
import gerlach.mtg_deck_generator.database.DbInitializerService;
...
public class MtgDeckGeneratorApplication { //implements CommandLineRunner {
	...
	@Bean
	public ApplicationRunner initializeDatabase(DbInitializerService initializerService,
												@Value("${db.initialize:false}") boolean initialize) {
		return args -> {
			if (initialize) {
				initializerService.initializeDb();
			}
		};
	}
